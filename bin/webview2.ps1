...
$Manifest = @{
    ...
    "architecture" = @{
        "64bit" = @{"url" = "$X64Uri#/$X64Path"; "hash" = $X64Hash }
        "32bit" = @{"url" = "$X86Uri#/$X86Path"; "hash" = $X86Hash }
        "arm64" = @{"url" = "$ARM64Uri#/$ARM64Path"; "hash" = $ARM64Hash }
    }
    "installer"    = @{
        "script" = 
        @(
            '$path = "$dir\$([System.IO.Path]::GetFileNameWithoutExtension($fname))"',
            'Move-Item "$path\*" "$dir" -Force', 'Remove-Item "$path" -Recurse -Force',
